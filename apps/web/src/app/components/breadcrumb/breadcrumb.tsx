...
export function BreadcrumbComponent({ title }: IBredcrumbInterface) {
  ...
  return (
    <nav className="breadcrumb-nav backdrop-blur-md bg-white/80 rounded-lg" aria-label="Fil d'Ariane">
      <div className="breadcrumb-container">
        <ol className="breadcrumb-list">
          ...
          {pathnames.map((name, index) => {
            ...
            return isLast ? (
              <li key={useId()} className="breadcrumb-item active" aria-current="page">
                {displayText}
              </li>
            ) : (
              <li key={useId()} className="breadcrumb-item">
